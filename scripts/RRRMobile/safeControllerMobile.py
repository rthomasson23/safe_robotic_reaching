...
from CBF_RRR_mobile_lineBarrier import *
...
class SafeController(object):
    ...
    def __init__(self, pb, bodyUniqueID, jointIDs):
        ...
        self.ID_P0, self.ID_P1, self.ID_R0, self.ID_R1, self.ID_R2 = jointIDs
    ...
    def computeSafeControl(self, maxForce, tau_nom):
        ...
        contacts_link0 = self._pb.getContactPoints(bodyA=self.bodyID, linkIndexA=self.ID_R0)
        contacts_link1 = self._pb.getContactPoints(bodyA=self.bodyID, linkIndexA=self.ID_R1)
        contacts_link2 = self._pb.getContactPoints(bodyA=self.bodyID, linkIndexA=self.ID_R2)
        ...
        if len(contacts_link0) > 0:
            ...
            pcx = pcontact[0]
            pcy = pcontact[1]
            lx, ly, _ = self.transform_world2link(pcontact, self.ID_R0)
            lx = np.array([lx, 0, 0])
            ly = np.array([ly, 0, 0])
        ...
        if len(contacts_link1) > 0:
            ...
            pcx = pcontact[0]
            pcy = pcontact[1]
            lx, ly, _ = self.transform_world2link(pcontact, self.ID_R1)
            lx = np.array([0, lx, 0])
            ly = np.array([0, ly, 0])
        ...
        if len(contacts_link2) > 0:
            ...
            lx, ly, _ = self.transform_world2link(pcontact, self.ID_R2)
            lx = np.array([0, 0, lx])
            ly = np.array([0, 0, ly])
